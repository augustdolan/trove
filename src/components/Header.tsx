import HamburgerMenu from "~/utils/HambugerMenu";
import { useIsAboveTailwindSm } from "~/utils/checkMediaQuery";

const Header = ({ username }: { username?: string }) => {
  const isAboveTailwindSm = useIsAboveTailwindSm();
  return (
    <header className="fixed z-10 flex w-full bg-green-500 p-4">
      {!isAboveTailwindSm && <HamburgerMenu />}
      <svg
        className="w-32"
      ...
      {username && (
        <p className="absolute right-5 top-5 capitalize">Hey {username}!</p>
      )}
    </header>
  );
};
